...
export default function PresaleList(props) {
  ...
  const { presalecount, listcount, presaleinfo, load4presale, load20presale, personalShow, search, filter_presale } =
    props;
    ...
    <div>
      <div>
        <GridContainer justifyContent="center">
          <Primary>
            <b>Decentralized Launchpad With Instant Listing And Liquidity Locking</b>
          </Primary>
        </GridContainer>
        ...
        <GridContainer justifyContent="center">
          ...
          <GridItem xs={12} sm={12} md={6}>
            <Box sx={{ display: 'flex', alignItems: 'flex-end' }}>
              <TextField
                // sx={{ input: { color: '#65B3AE' } }}
                color="error"
                fullWidth
                placeholder="Input Presale Address"
                id="presale_address"
                variant="standard"
              />
          ...
          <FormControl component="fieldset">
            ...
            <RadioGroup row aria-label="position" name="position" defaultValue="top">
              <FormControlLabel
                value="step1"
                control={
                  <Radio size="small" {...controlProps('entire')} sx={{ '&.Mui-checked': { color: col_step1 } }} />
                }
                label="Entire"
                labelPlacement="end"
              />
              <FormControlLabel
                value="step1"
                control={
                  <Radio size="small" {...controlProps('active')} sx={{ '&.Mui-checked': { color: col_step1 } }} />
                }
                label="Active"
                labelPlacement="end"
              />
              <FormControlLabel
                value="step2"
                control={
                  <Radio size="small" {...controlProps('success')} sx={{ '&.Mui-checked': { color: col_step2 } }} />
                }
                label="Success"
                labelPlacement="end"
              />
              <FormControlLabel
                value="step3"
                control={
                  <Radio size="small" {...controlProps('failed')} sx={{ '&.Mui-checked': { color: col_step3 } }} />
                }
                label="Failed"
                labelPlacement="end"
              />
              ...
              return (
                <GridItem key={index} xs={12} sm={12} md={3}>
                  <Thumbnail
                    ...
                    raised={thumb.raised}
                    hardcap={thumb.hardcap}
                    softcap={thumb.softcap}
                    mincap={thumb.min}
                    maxcap={thumb.max}
                    liquidity={thumb.liquidity}
                    flagstr={thumb.flagstr}
                    timestr={thumb.timestr}
                    time={thumb.time}
                    tokenname={thumb.token_name}
                    tokensymbol={thumb.token_symbol}
                    index={thumb.index}
      ...
      <div>
        <GridContainer justifyContent="center">
          ...
          <GridItem xs={12} sm={12} md={3}>
            <h4>
              {listcount}/{parseInt(presalecount)}
            </h4>
          </GridItem>
          <GridItem xs={12} sm={12} md={3}>
